...
def compute_Phi(x,p):
    ...
    #want an nxp matrix where each element i,j is x[i] ^ j

    #make an empty matrix size nxp
    Phi = np.zeros((x.shape[0], p))

    # fill in the matrix using the equation
    for j in range(p):
        Phi[:, j] = x ** j
...
def compute_yhat(Phi, w):
    ...
    #need to calculate the dot product of Phi and w
    yhat = np.dot(Phi, w)
...
def compute_L(yhat,y):
    ...
    #MSE = mean(yhat - y) ^2 --> then divide by 2
...
def compute_dL_dw(y, yhat, Phi):
    ...
    ## INSERT YOUR CODE HERE
     ...
     # 1/n * Phi * (yhat - y)
    dL_dw = (1/y.shape[0]) * np.dot(Phi.T, (yhat-y)) #need to transpose Phi to get right shape for dot product
...
def update_w(w, dL_dw, alpha = 0.001):
    ...
    # w - alpha * 1/n* sum((yhat - y) x)
    # dL_dw = 1/n* sum((yhat - y) x)
    w = w - (alpha * dL_dw)
...
def train(X, Y, alpha=0.001, n_epoch=100):
    ...
    # Back propagation: compute local gradients 
        Phi = X  #since X is a feature matrix we can use it for Phi
